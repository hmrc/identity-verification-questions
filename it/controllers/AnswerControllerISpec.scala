  ...
  "POST /answers" should {
    ...
    "return 200 with score unknown" when {
      ...
      "no questions found for requested origin" in new SetUp {
        questionRepository.store(questionDataCache(Origin("wrong_origin"), Seq(ninoIdentifier)))
        val response: WSResponse = await(resourceRequest(journeyPath).post(Json.toJson(answerCheck)))
        response.status shouldBe 200
        response.json.validate[List[QuestionResult]] shouldBe JsSuccess(List(questionResultUnknown))
      }
      ...
      "no questions found for requested identifier" in new SetUp {
        questionRepository.store(questionDataCache(origin, Seq(utrIdentifier)))
        val response: WSResponse = await(resourceRequest(journeyPath).post(Json.toJson(answerCheck)))
        response.status shouldBe 200
        response.json.validate[List[QuestionResult]] shouldBe JsSuccess(List(questionResultUnknown))
      }
    ...
    "return 200 with score correct" when {
      "answer matches correct answer stored against origin and identifier" in new SetUp {
        questionRepository.store(questionDataCache(origin, Seq(ninoIdentifier)))
        val response: WSResponse = await(resourceRequest(journeyPath).post(Json.toJson(answerCheck)))
        response.status shouldBe 200
    ...
    trait SetUp{
      ...
      val answerCheck: AnswerCheck = AnswerCheck(origin, Seq(ninoIdentifier), answerDetails)
      val incorrectAnswerDetails: Seq[AnswerDetails] = Seq(AnswerDetails(PaymentToDate, StringAnswer("666.00")))
      val incorrectAnswerCheck: AnswerCheck = AnswerCheck(origin, Seq(ninoIdentifier), incorrectAnswerDetails)
      ...
      def questionDataCache(origin: Origin, identifiers: Seq[Identifier]) = QuestionDataCache(Selection(origin, identifiers), questions, LocalDateTime.now() plusMinutes(1))
