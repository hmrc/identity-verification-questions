...
class QuestionControllerSpec extends UnitSpec with LogCapturing {
  ...
  "POST /questions" should {
    "return 200 ok" in new Setup {
      (fakeEvidenceRetrievalService.callAllEvidenceSources(_: Selection, _:String)(using _:Request[?],_: HeaderCarrier)).expects(*, *, *, *).returning(Future.successful(questionResponse))
